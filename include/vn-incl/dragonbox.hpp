...
//   #undef-ed at the end of this header if JKJ_DRAGONBOX_LEAK_MACROS is not defined.
...
#if !defined(JKJ_HEADER_DRAGONBOX) || (defined(JKJ_DRAGONBOX_LEAK_MACROS) && !defined(JKJ_DRAGONBOX_MACROS_DEFINED))
	...
	// no impact on the codegen if compact cache for IEEE-754 binary32 is not used.
		...
		#if defined(__x86_64__) || defined(__aarch64__) || defined(__ppc64__) || defined(_M_X64) || defined(_M_ARM64)
	...
	namespace dragonbox {
		...
		namespace detail {
			namespace stdr {
				...
				template<class T> using make_unsigned = JKJ_STD_REPLACEMENT_NAMESPACE::make_unsigned<T>;
			}
		}
		...
		namespace detail {
			...
			template<class T, stdr::size_t N> struct array {
				...
				JKJ_CONSTEXPR14 T& operator[](stdr::size_t idx) noexcept {
					return data_[idx];
				}
			};
			...
			template<class T> JKJ_CONSTEXPR20 T read_static_data(T const* ptr) noexcept {
				JKJ_IF_CONSTEVAL {
					return *ptr;
				}
				else {
		...
		template<class Float> struct default_float_bit_carrier_conversion_traits {
			...
			static_assert(detail::stdr::numeric_limits<Float>::is_iec559 && detail::stdr::numeric_limits<Float>::radix == 2 &&
		...
		namespace detail {
			...
			namespace wuint {
				...
				struct uint32_64 {
					...
					constexpr uint32_64(stdr::uint_least32_t high32, stdr::uint_least64_t low64) noexcept : low64_{ low64 }, high32_{ high32 } {
					}

					constexpr stdr::uint_least32_t high32() const noexcept {
						return high32_;
					}
					constexpr stdr::uint_least64_t low64() const noexcept {
						return low64_;
					}
				};
				...
				struct uint64_32 {
					...
					constexpr uint64_32(stdr::uint_least64_t high64, stdr::uint_least32_t low32) noexcept : high64_{ high64 }, low32_{ low32 } {
					}

					constexpr stdr::uint_least64_t high64() const noexcept {
						return high64_;
					}
					constexpr stdr::uint_least32_t low32() const noexcept {
						return low32_;
					}
				};
				...
				JKJ_SAFEBUFFERS inline JKJ_CONSTEXPR20 uint128 umul128(stdr::uint_least64_t x, stdr::uint_least64_t y) noexcept {
				...
				// integers.
				JKJ_SAFEBUFFERS inline JKJ_CONSTEXPR20 stdr::uint_least64_t umul128_upper64(stdr::uint_least64_t x, stdr::uint_least64_t y) noexcept {
				...
				// unsigned integer.
				inline JKJ_CONSTEXPR20 stdr::uint_least64_t umul96_upper64(stdr::uint_least32_t x, stdr::uint_least64_t y) noexcept {
	#if defined(__SIZEOF_INT128__) || (defined(_MSC_VER) && defined(_M_X64))
			...
			namespace log {
				static_assert((stdr::int_fast32_t(-1) >> 1) == stdr::int_fast32_t(-1) && (stdr::int_fast16_t(-1) >> 1) == stdr::int_fast16_t(-1),
				...
				template<template<stdr::size_t> class Info, stdr::int_least32_t min_exponent, stdr::int_least32_t max_exponent, stdr::size_t current_tier = 0,
				...
				template<stdr::size_t tier> struct floor_log2_pow5_info;
				template<> struct floor_log2_pow5_info<0> {
					using default_return_type = stdr::int_fast8_t;
					// 24-bits are enough in fact.
				...
				template<> struct floor_log2_pow5_info<1> {
					using default_return_type						  = stdr::int_fast16_t;
				...
				template<stdr::int_least32_t min_exponent = -642, stdr::int_least32_t max_exponent = 642,
					class ReturnType = typename compute_impl<floor_log2_pow5_info, min_exponent, max_exponent>::default_return_type, class Int>
				constexpr ReturnType floor_log2_pow5(Int e) noexcept {
					return compute_impl<floor_log2_pow5_info, min_exponent, max_exponent>::template compute<ReturnType>(e);
				}
			...
			namespace div {
				...
				template<int N, class UInt, UInt n_max> JKJ_CONSTEXPR20 UInt divide_by_pow10(UInt n) noexcept {
					...
					JKJ_IF_CONSTEXPR(stdr::is_same<UInt, stdr::uint_least32_t>::value && N == 1 && n_max <= UINT32_C(1073741828)) {
					...
					else JKJ_IF_CONSTEXPR(stdr::is_same<UInt, stdr::uint_least64_t>::value && N == 1 && n_max <= UINT64_C(4611686018427387908)) {
					...
					else JKJ_IF_CONSTEXPR(stdr::is_same<UInt, stdr::uint_least64_t>::value && N == 3 && n_max <= UINT64_C(15534100272597517998)) {
		...
		template<class Dummy> struct compressed_cache_holder<ieee754_binary32, Dummy> {
			...
			static constexpr cache_holder_t cache JKJ_STATIC_DATA_SECTION = make_cache(detail::make_index_sequence<compressed_table_size>{});
			...
			template<class ShiftAmountType, class DecimalExponentType> static JKJ_CONSTEXPR20 cache_entry_type get_cache(DecimalExponentType k) noexcept {
				...
				using unsigned_exponent_type = typename detail::stdr::make_unsigned<DecimalExponentType>::type;
				auto const cache_index		 = DecimalExponentType(detail::stdr::uint_fast16_t(unsigned_exponent_type(k - min_k) * detail::stdr::uint_fast16_t(79)) >> 10);
				...
				if (offset == 0) {
					...
					auto const alpha = ShiftAmountType(detail::log::floor_log2_pow5<min_k, max_k>(k) - detail::log::floor_log2_pow5<min_k, max_k>(kb));
					assert(alpha > 0 && alpha < 32);

					// Try to recover the real cache.
					constexpr auto pow5_6 = detail::stdr::uint_fast32_t(pow5_table[6]);
					auto const pow5		  = offset >= 7 ? detail::stdr::uint_fast32_t(pow5_6 * detail::read_static_data(pow5_table.data() + offset - 6))
														: detail::stdr::uint_fast32_t(detail::read_static_data(pow5_table.data() + offset));
					auto mul_result		  = detail::wuint::umul96(pow5, base_cache);

					auto const recovered_cache =
	#if JKJ_FAST_MUL64
						cache_entry_type((((detail::stdr::uint_least64_t(mul_result.high32()) << ShiftAmountType(64 - alpha)) | (mul_result.low64() >> alpha)) + 1) &
							UINT64_C(0xffffffffffffffff));
	#else
						cache_entry_type((((mul_result.high64() << ShiftAmountType(32 - alpha)) | (mul_result.low32() >> alpha)) + 1) & UINT64_C(0xffffffffffffffff));
	#endif
		...
		template<class Dummy> struct compressed_cache_holder<ieee754_binary64, Dummy> {
			...
			template<class ShiftAmountType, class DecimalExponentType> static JKJ_CONSTEXPR20 cache_entry_type get_cache(DecimalExponentType k) noexcept {
				...
				static_assert(max_k - min_k <= (detail::stdr::numeric_limits<DecimalExponentType>::max)(), "");
				using unsigned_exponent_type = typename detail::stdr::make_unsigned<DecimalExponentType>::type;
				auto const cache_index		 = DecimalExponentType(detail::stdr::uint_fast32_t(unsigned_exponent_type(k - min_k) * detail::stdr::uint_fast32_t(607)) >> 14);
				auto const kb				 = DecimalExponentType(cache_index * compression_ratio + min_k);
				auto const offset			 = DecimalExponentType(k - kb);
				...
				if (offset == 0) {
					...
					auto const high_to_middle = detail::stdr::uint_least64_t((recovered_cache.high() << ShiftAmountType(64 - alpha)) & UINT64_C(0xffffffffffffffff));
					auto const middle_to_low  = detail::stdr::uint_least64_t((recovered_cache.low() << ShiftAmountType(64 - alpha)) & UINT64_C(0xffffffffffffffff));
		...
		namespace policy {
			...
			namespace cache {
				JKJ_INLINE_VARIABLE struct full_t {
					...
					static JKJ_CONSTEXPR20 typename cache_holder_type<FloatFormat>::cache_entry_type get_cache(DecimalExponentType k) noexcept {
						assert(k >= cache_holder_type<FloatFormat>::min_k && k <= cache_holder_type<FloatFormat>::max_k);

						return detail::read_static_data(cache_holder_type<FloatFormat>::cache.data() + detail::stdr::size_t(k - cache_holder_type<FloatFormat>::min_k));
					}
				} full = {};
				...
				JKJ_INLINE_VARIABLE struct minimal_t {
					...
					template<class FormatTraits, detail::stdr::int_least32_t lower_bound, detail::stdr::uint_least32_t upper_bound> using decimal_exponent_type =
						typename detail::stdr::conditional<lower_bound >= (detail::stdr::numeric_limits<detail::stdr::int_least8_t>::min)() &&
							...
							typename detail::stdr::conditional<lower_bound >= (detail::stdr::numeric_limits<detail::stdr::int_least16_t>::min)() &&
									upper_bound <= (detail::stdr::numeric_limits<detail::stdr::int_least16_t>::max)(),
								detail::stdr::int_least16_t,
								typename detail::stdr::conditional<lower_bound >= (detail::stdr::numeric_limits<detail::stdr::int_least32_t>::min)() &&
		...
		struct remove_trailing_zeros_traits<policy::trailing_zero::remove_t, ieee754_binary64, detail::stdr::uint_least64_t, DecimalExponentType> {
			JKJ_FORCEINLINE static JKJ_CONSTEXPR14 void remove_trailing_zeros(detail::stdr::uint_least64_t& significand, DecimalExponentType& exponent) noexcept {
				...
				r			= detail::bits::rotr<64>(detail::stdr::uint_least64_t(significand * UINT64_C(14757395258967641293)), 1);
		...
		struct remove_trailing_zeros_traits<policy::trailing_zero::remove_compact_t, ieee754_binary64, detail::stdr::uint_least64_t, DecimalExponentType> {
			JKJ_FORCEINLINE static JKJ_CONSTEXPR14 void remove_trailing_zeros(detail::stdr::uint_least64_t& significand, DecimalExponentType& exponent) noexcept {
				// See https://github.com/jk-jeon/rtz_benchmark.
				while (true) {
					auto const r = detail::stdr::uint_least64_t(significand * UINT64_C(5534023222112865485));
		...
		struct multiplication_traits<ieee754_binary_traits<ieee754_binary32, detail::stdr::uint_least32_t, ExponentInt>, detail::stdr::uint_least64_t, 64>
			...
			static JKJ_CONSTEXPR20 compute_mul_result compute_mul(carrier_uint u, cache_entry_type const& cache) noexcept {
				auto const r = detail::wuint::umul96_upper64(u, cache);
				return { carrier_uint(r >> 32), carrier_uint(r) == 0 };
			}

			template<class ShiftAmountType> static constexpr detail::stdr::uint_least64_t compute_delta(cache_entry_type const& cache, ShiftAmountType beta) noexcept {
				return detail::stdr::uint_least64_t(cache >> ShiftAmountType(cache_bits - 1 - beta));
			}

			template<class ShiftAmountType>
			static JKJ_CONSTEXPR20 compute_mul_parity_result compute_mul_parity(carrier_uint two_f, cache_entry_type const& cache, ShiftAmountType beta) noexcept {
				assert(beta >= 1);
				assert(beta <= 32);
			...
			template<class ShiftAmountType>
			static constexpr carrier_uint compute_left_endpoint_for_shorter_interval_case(cache_entry_type const& cache, ShiftAmountType beta) noexcept {
				return carrier_uint((cache - (cache >> (significand_bits + 2))) >> ShiftAmountType(cache_bits - significand_bits - 1 - beta));
			}

			template<class ShiftAmountType>
			static constexpr carrier_uint compute_right_endpoint_for_shorter_interval_case(cache_entry_type const& cache, ShiftAmountType beta) noexcept {
				return carrier_uint((cache + (cache >> (significand_bits + 1))) >> ShiftAmountType(cache_bits - significand_bits - 1 - beta));
			}

			template<class ShiftAmountType> static constexpr carrier_uint compute_round_up_for_shorter_interval_case(cache_entry_type const& cache, ShiftAmountType beta) noexcept {
		...
		namespace detail {
			...
			template<class FormatTraits> struct impl : private FormatTraits::format {
				...
				static constexpr int min_(int x, int y) noexcept {
					return x < y ? x : y;
				}
				static constexpr int max_(int x, int y) noexcept {
					return x > y ? x : y;
				}

				static constexpr int min_k =
					min_(-log::floor_log10_pow2_minus_log10_4_over_3(max_exponent - significand_bits), -log::floor_log10_pow2(max_exponent - significand_bits) + kappa);

				// We do invoke shorter_interval_case for exponent == min_exponent case,
				// so we should not add 1 here.
				static constexpr int max_k =
					max_(-log::floor_log10_pow2_minus_log10_4_over_3(min_exponent - significand_bits /*+ 1*/), -log::floor_log10_pow2(min_exponent - significand_bits) + kappa);
				...
				template<class PreferredIntegerTypesPolicy> using decimal_exponent_type = typename PreferredIntegerTypesPolicy::template decimal_exponent_type<FormatTraits,
					detail::stdr::int_least32_t(min_(-max_k, min_k)), detail::stdr::int_least32_t(max_(max_k, -min_k + kappa + 1))>;
				...
				JKJ_SAFEBUFFERS static JKJ_CONSTEXPR20 return_type<SignPolicy, TrailingZeroPolicy, PreferredIntegerTypesPolicy> compute_nearest(
					...
					if (binary_exponent != 0) {
						...
						if (two_fc == 0) {
							...
							// Compute k and beta.
							auto const minus_k = log::floor_log10_pow2_minus_log10_4_over_3<min_exponent - format::significand_bits, max_exponent - format::significand_bits,
								decimal_exponent_type_>(binary_exponent);
							auto const beta	   = shift_amount_type(binary_exponent + log::floor_log2_pow10<min_k, max_k>(decimal_exponent_type_(-minus_k)));

							// Compute xi and zi.
							auto const cache = CachePolicy::template get_cache<format, shift_amount_type>(decimal_exponent_type_(-minus_k));
					...
					auto const deltai = static_cast<remainder_type_>(multiplication_traits_::compute_delta(cache, beta));
					...
					// fine.
					auto const z_result = multiplication_traits_::compute_mul(carrier_uint((two_fc | 1) << beta), cache);
					...
					do {
						if (r < deltai) {
							...
							if ((r | remainder_type_(!z_result.is_integer) | remainder_type_(interval_type.include_right_endpoint())) == 0) {
								JKJ_IF_CONSTEXPR(BinaryToDecimalRoundingPolicy::tag == policy::binary_to_decimal_rounding::tag_t::do_not_care) {
							...
							auto const x_result = multiplication_traits_::compute_mul_parity(carrier_uint(two_fc - 1), cache, beta);
					...
					JKJ_IF_CONSTEXPR(BinaryToDecimalRoundingPolicy::tag == policy::binary_to_decimal_rounding::tag_t::do_not_care) {
						...
						if (divisible_by_small_divisor) {
							...
							// is an even number.
							auto const y_result = multiplication_traits_::compute_mul_parity(two_fc, cache, beta);
				...
				JKJ_FORCEINLINE JKJ_SAFEBUFFERS static JKJ_CONSTEXPR20 return_type<SignPolicy, TrailingZeroPolicy, PreferredIntegerTypesPolicy> compute_left_closed_directed(
					...
					do {
						if (r > deltai) {
							...
							auto const z_result = multiplication_traits_::compute_mul_parity(carrier_uint(two_fc + 2), cache, beta);
				...
				JKJ_FORCEINLINE JKJ_SAFEBUFFERS static JKJ_CONSTEXPR20 return_type<SignPolicy, TrailingZeroPolicy, PreferredIntegerTypesPolicy> compute_right_closed_directed(
					...
					auto const minus_k = decimal_exponent_type_(
						...
						kappa);
					auto const cache = CachePolicy::template get_cache<format, shift_amount_type>(decimal_exponent_type_(-minus_k));
					auto const beta	 = shift_amount_type(binary_exponent + log::floor_log2_pow10(decimal_exponent_type_(-minus_k)));

					// Compute zi and deltai.
					// 10^kappa <= deltai < 10^(kappa + 1)
					auto const deltai	  = static_cast<remainder_type_>(multiplication_traits_::compute_delta(cache, shift_amount_type(beta - (shorter_interval ? 1 : 0))));
					carrier_uint const zi = multiplication_traits_::compute_mul(carrier_uint(two_fc << beta), cache).integer_part;
