...
// Use the provided definition.
	...
	template<int num_bits> ZMIJ_INLINE auto to_digits(uint64_t value, const data& d) noexcept -> dec_digits<num_bits> {
	...
	template<typename Float, typename UInt> ZMIJ_INLINE auto to_decimal(UInt bin_sig, int64_t raw_exp, bool regular, const data& d) noexcept -> to_decimal_result {
		...
		int dec_exp					   = use_umul128_hi64 ? umul128_hi64(bin_exp, log10_2_sig << (64 - log10_2_exp)) : compute_dec_exp(bin_exp);
		ZMIJ_ASM(("" : "+r"(dec_exp)));// Force 32-bit reg for sxtw addressing.
		unsigned char shift = exp_shift_table::enable ? d.exp_shifts.data[bin_exp + float_traits<double>::exp_offset] : compute_exp_shift(bin_exp, dec_exp + 1) + extra_shift;
		...
		template<typename Float> auto write(Float value, char* buffer) noexcept -> char* {
			...
			*buffer = '-';
			...
			if (!is_normal) [[ZMIJ_UNLIKELY]] {
				...
				dec				  = zmij::to_decimal<Float>(bin_sig, 1, true, *d);
			...
			if (traits::max_exponent10 >= 100) {
				// digit = dec_exp / 100
				uint32_t digit = use_umul128_hi64 ? umul128_hi64(dec_exp, 0x290000000000000) : (uint32_t(dec_exp) * div100_sig) >> div100_exp;
				*buffer		   = '0' + digit;
				buffer += dec_exp >= 100;
				dec_exp -= digit * 100;
			}
